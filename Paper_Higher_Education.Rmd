...
bar = ggplot(data=x, aes(x=reorder(factor(Region),Total_Egresados),y=Total_Egresados, fill=Total_Egresados))  + coord_flip() + theme_minimal() + geom_bar(stat="identity") + labs(x="Region",y="Graduates") + theme(legend.position = "none", axis.title = element_text(size=17), axis.text.y = element_text(size=14), axis.text.x = element_text(size=14)) + geom_text(aes(label = Porc), position = position_dodge(width= 1), vjust= 0, hjust = -0.05, size = 6, color="black") + scale_y_continuous(expand = expansion(mult = c(0, .1)))+scale_fill_distiller(palette = "RdBu", direction=1,limits=c(0,920000),breaks = seq(0,920000,150000)) 
...
graf_total = chile_1 + theme_bw() + theme(legend.position = "none", axis.title = element_text(size=15)) +scale_fill_distiller(palette = "RdBu", direction=1, limits=c(0,920000), breaks = seq(0,920000,150000)) 
...
graf_total2 = chile_2 + theme_bw() + theme(legend.position = "right", axis.title = element_text(size=15), legend.text = element_text(size=12), legend.title = element_text(size=15)) +scale_fill_distiller(palette = "RdBu",direction=1,limits=c(0,920000), breaks = seq(0,920000,150000)) 
...
todos <- colegios %>% group_by(AGNO_EGRESO) %>% summarise(No_Matriculado=sum(No_Matriculado),Matriculado=sum(Matriculado)) %>% ungroup() %>% mutate(Tasa=Matriculado/(Matriculado+No_Matriculado)*100)
todos$COD_DEPE2="All"

final <- bind_rows(municipales,subv,pagados,todos)
...
Fig_5 = ggplot(final, aes(x=AGNO_EGRESO, y=Tasa, group=factor(COD_DEPE2))) +
  geom_line(aes(color=factor(COD_DEPE2))) +
  geom_point(aes(color=factor(COD_DEPE2)))+ 
  labs(x="Admission Process", y="Enrolment Rate", color="Dependence") +
  ...
  theme_bw() +
  geom_rect(aes(xmin = 2007, xmax = 2010.5, ymin = -Inf, ymax = Inf),fill = "red", alpha = 0.005) +
  geom_rect(aes(xmin = 2010.5, xmax = 2014.5, ymin = -Inf, ymax = Inf),fill = "blue", alpha = 0.005) + 
  geom_rect(aes(xmin = 2014.5, xmax = 2018.5, ymin = -Inf, ymax = Inf),fill = "red", alpha = 0.005) +
  geom_rect(aes(xmin = 2018.5, xmax = 2020.8, ymin = -Inf, ymax = Inf),fill = "blue", alpha = 0.005) + 
  ...
  geom_text(aes(x=2015.6, label="Gratuidad", y=20), colour="blue", angle=90, text=element_text(size=1)) + 
  geom_vline(xintercept = 2011.8, linetype="dashed") +
  geom_text(aes(x=2011.6, label="Students' Movement", y=20), colour="red", angle=90, text=element_text(size=1)) + 
  geom_vline(xintercept = 2009.8, linetype="dashed") + 
  geom_text(aes(x=2009.6, label="LGE", y=20), colour="blue", angle=90, text=element_text(size=1)) + 
  geom_vline(xintercept = 2006.8) + 
  geom_vline(xintercept = 2019.8, linetype="dashed") + 
  geom_text(aes(x=2019.6, label="Social Outbreak", y=20), colour="red", angle=90, text=element_text(size=1)) +
  geom_text(aes(label = Porc), vjust= -0.5, hjust = 0, size = 4, color="black", check_overlap = TRUE, nudge_y = 0.5)
...
Fig_2 = ggplot(final, aes(x=AGNO_EGRESO, y=Total_Egresados, group=factor(COD_DEPE2))) +
  geom_line(aes(color=factor(COD_DEPE2)))+
  geom_point(aes(color=factor(COD_DEPE2)))+ 
  labs(x="Admission Process", y="Graduates", color="Dependence") +
  ...
  theme(axis.title.y.right = element_blank(),plot.title = element_text(size = 12, face = "bold"),
        ...
        legend.position = "top") +
  geom_vline(xintercept = 2015.8) + 
  geom_text(aes(x=2015.6, label="Gratuidad", y=40000), colour="blue", angle=90, text=element_text(size=1)) + 
  geom_vline(xintercept = 2011.8) + 
  geom_text(aes(x=2011.6, label="St.Movement", y=40000), colour="red", angle=90, text=element_text(size=1)) +
  geom_vline(xintercept = 2009.8) + 
  geom_text(aes(x=2009.6, label="LGE", y=40000), colour="blue", angle=90, text=element_text(size=1)) + 
  geom_vline(xintercept = 2006.8) + 
  geom_vline(xintercept = 2019.8) + 
  geom_text(aes(x=2019.6, label="Soc.Outbreak", y=40000), colour="red", angle=90, text=element_text(size=1)) +
  geom_text(aes(label = Porc), vjust= -0.5, hjust = 0, size = 4, color="black", check_overlap = TRUE, nudge_y = 0.5) 
...
Fig_4 = ggplot(data=estudiantes, aes(x=COD_DEPE2, y=NEM, fill=factor(GEN_ALU))) +
  ...
  theme(plot.title = element_text(size = 12, face = "bold"),legend.title=element_text(size=16, angle=0),
        legend.text=element_text(size=14, angle = 0), axis.title.x = element_text(size = 18, vjust = -0.5), 
        axis.text.y = element_text(size=13, angle = 0), axis.title.y = element_text(size=18, angle=90), 
        axis.text.x = element_text(size=14, angle=0), axis.title.y.right = element_blank(), 
        legend.position = "top") + 
...
```{r Fig: 14, echo=FALSE, fig.align="center", fig.height=5, fig.width=10, message=FALSE, warning=FALSE}
setwd("~/Documents/GitHub/HE_Chile")
db_final <- read_rds("BASE_PAPER.RDS")
db_final <- db_final %>% filter(JOVEN=="1")
db_final <- db_final %>% mutate(COD_ENSE=ifelse(COD_ENSE<400,"H-C","T-P")) #%>% filter(COD_ENSE=="H-C")
...
Fig_14 = ggplot(colegios, aes(x=AGNO_EGRESO, y=Tasa, group=factor(COD_DEPE2))) +
  geom_line(aes(color=factor(COD_DEPE2))) +
  geom_point(aes(color=factor(COD_DEPE2))) + 
  labs(x="Admission Process", y="Enrolment Rate", color="Dependence") +
  ...
  theme(plot.title = element_text(size = 12, face = "bold"),legend.title=element_text(size=16, angle=0),
        ...
        axis.text.x = element_text(size=14, angle=90), axis.title.y.right = element_blank(), strip.text = element_text(size = 20),
        legend.position="top") +
  geom_text(aes(label = Porc), vjust= -0.5, hjust = 0, size = 4, color="black", check_overlap = TRUE, nudge_y = 0.5) + 
  facet_rep_wrap(~PERCENTIL, nrow=2,repeat.tick.labels = T) 
  
Fig_14

ggsave("/Users/bolea/Documents/GitHub/HE_Chile/Figures/Fig_14.eps",Fig_14, width=23.67, height=12.6, limitsize = FALSE)
ggsave("/Users/bolea/Documents/GitHub/HE_Chile/Figures/Fig_14.pdf",Fig_14, width=23.67, height=12.6, limitsize = FALSE)
...
Fig_6 = ggplot(colegios, aes(x=AGNO_EGRESO, y=Tasa, group=factor(GEN_ALU))) +
  geom_line(aes(color=factor(GEN_ALU))) +
  geom_point(aes(color=factor(GEN_ALU))) +
  theme() + 
  labs(x="Admission Process", y="Enrolment Rate", color="Sex") +
...
Alluvial <- Alluvial %>% mutate(TIPO_INST_3=ifelse(is.na(TIPO_INST_3),"No-Matriculado",TIPO_INST_3))
Alluvial <- Alluvial %>% mutate(OECD_AREA=ifelse(is.na(TIPO_INST_3),"No-Matriculado",OECD_AREA))
...
Fig_7 <- ggplot(as.data.frame(Alluvial),
  ...
  scale_fill_brewer(type = "qual", palette = "Set1") + labs(y="Graduates enrolled", fill="Dependence") +
...
```{r Fig: 11, echo=FALSE, fig.height=7, fig.width=15, message=FALSE, warning=FALSE}
Data_2019 <- read_rds("BASE_PAPER.RDS")
Data_2019 <- Data_2019 %>% filter(JOVEN=="1")
Data_2019 <- Data_2019 %>% rename(COD_REG_ALU=codigo_region,Region=nombre_region)

df <- Data_2019 %>% filter(!is.na(COD_REG_ALU)) %>% filter(COD_REG_ALU>0) 
...
```{r Fig: 12 and 13, echo=FALSE, fig.height=7, fig.width=15, message=FALSE, warning=FALSE}

Data_2019 <- read_rds("BASE_PAPER.RDS")
Data_2019 <- Data_2019 %>% filter(JOVEN=="1")
Data_2019 <- Data_2019 %>% rename(COD_REG_ALU=codigo_region,Region=nombre_region)
 
#2007-2011 #remember admission process = agno_egreso + 1

matriculado_region <- Data_2019 %>% 
  filter(JOVEN=="1",AGNO_EGRESO<2013) %>%
...
bar = ggplot(data=x, aes(x=reorder(factor(Region),Tasa_Ingreso),y=Tasa_Ingreso, fill=Tasa_Ingreso))  + coord_flip() + theme_minimal() + geom_bar(stat="identity") + labs(x="Region",y="Enrolment Rate") + theme(legend.position = "none", axis.title = element_text(size=17), axis.text.y = element_text(size=14), axis.text.x = element_text(size=14)) + geom_text(aes(label = Porc), position = position_stack(vjust = 0.5), size = 5, color="black") + scale_y_continuous(expand = expansion(mult = c(0, .1)))+scale_fill_distiller(palette = "RdBu", direction=1,limits=c(34,57)) 

chile_1 = ggplot() +  geom_sf(data = x %>% filter(Zoom==1), aes(fill=Tasa_Ingreso, geometry=geometry)) +  geom_sf_label_repel(data=x %>% filter(Zoom==1),aes(label = Region), nudge_x = -3, nudge_y = 0, seed = 10 ,colour="black", size=5) + coord_sf(xlim = c( -77.1,-65), ylim = c(-38, -17))
chile_1 = chile_1 + theme(legend.position="right", legend.box = "vertical") 
chile_1 = chile_1 + xlab("Longitude") + ylab("Latitude") + annotation_scale() + annotation_north_arrow(location='tr')
chile_1 = chile_1 + theme(plot.title = element_text(hjust = 0.5))  # + theme(legend.title=element_blank()) 
graf_total = chile_1 + theme_bw() + theme(legend.position = "none", axis.title = element_text(size=15)) +scale_fill_distiller(palette = "RdBu", direction=1,limits=c(34,57)) 

chile_2 = ggplot() +  geom_sf(data = x %>% filter(Zoom==0), aes(fill=Tasa_Ingreso, geometry=geometry)) +  geom_sf_label_repel(data=x %>% filter(Zoom==0),aes(label = Region), nudge_x = 3, nudge_y = 0, seed = 10 ,colour="black", size=5) + coord_sf(xlim = c( -77.1,-65), ylim = c(-57, -38))
chile_2 = chile_2 + theme(legend.position="right", legend.box = "vertical") + labs(fill="Enrolment Rate")
chile_2 = chile_2 + xlab("Longitude") + ylab("Latitude") + annotation_scale() + annotation_north_arrow(location='tr')
chile_2 = chile_2 + theme(plot.title = element_text(hjust = 0.5),axis.title = element_text(size=15), legend.text = element_text(size=15)) + scale_fill_distiller(palette = "Spectral",limits=c(34,57))  # + theme(legend.title=element_blank()) 
graf_total2 = chile_2 + theme_bw() + theme(legend.position = "right", axis.title = element_text(size=15), legend.text = element_text(size=12), legend.title = element_text(size=15)) +scale_fill_distiller(palette = "RdBu", direction=1,limits=c(34,57)) 

Fig_12 <- grid.arrange(bar,chile_1,chile_2, ncol=3, nrow=1)

ggsave("/Users/bolea/Documents/GitHub/HE_Chile/Figures/Fig_12.eps",Fig_12, width=19.20, height=10.57, limitsize = FALSE)
ggsave("/Users/bolea/Documents/GitHub/HE_Chile/Figures/Fig_12.pdf",Fig_12, width=19.20, height=10.57, limitsize = FALSE)



#2012-2019

matriculado_region <- Data_2019 %>%  filter(JOVEN=="1",AGNO_EGRESO>=2013) %>% group_by(COD_REG_ALU, Region, MATRICULADO, TIPO_INST_1) %>% summarise(N=n()) #19992 alumnos sin region de origen.
matriculado_region=dcast(matriculado_region,COD_REG_ALU + Region ~ TIPO_INST_1 + MATRICULADO)
matriculado_region <- matriculado_region %>% rename(Universidades=Universidades_1 , CFT=`Centros de Formación Técnica_1` ,IP=`Institutos Profesionales_1`)
matriculado_region <- matriculado_region %>% rename(No_Matriculados=NA_0)
matriculado_region <- matriculado_region %>% mutate(Total_Egresados=Universidades+CFT+IP+No_Matriculados)
...
bar = ggplot(data=x, aes(x=reorder(factor(Region),Tasa_Ingreso),y=Tasa_Ingreso, fill=Tasa_Ingreso))  + coord_flip() + theme_minimal() + geom_bar(stat="identity") + labs(x="Region",y="Enrolment Rate") + theme(legend.position = "none", axis.title = element_text(size=17), axis.text.y = element_text(size=14), axis.text.x = element_text(size=14)) + geom_text(aes(label = Porc), position = position_stack(vjust = 0.5), size = 5, color="black") + scale_y_continuous(expand = expansion(mult = c(0, .1)))+scale_fill_distiller(palette = "RdBu", direction=1,limits=c(45,65)) 

chile_1 = ggplot() +  geom_sf(data = x %>% filter(Zoom==1), aes(fill=Tasa_Ingreso, geometry=geometry)) +  geom_sf_label_repel(data=x %>% filter(Zoom==1),aes(label = Region), nudge_x = -3, nudge_y = 0, seed = 10 ,colour="black", size=5) + coord_sf(xlim = c( -77.1,-65), ylim = c(-38, -17))
chile_1 = chile_1 + theme(legend.position="right", legend.box = "vertical") 
chile_1 = chile_1 + xlab("Longitude") + ylab("Latitude") + annotation_scale() + annotation_north_arrow(location='tr')
chile_1 = chile_1 + theme(plot.title = element_text(hjust = 0.5))  # + theme(legend.title=element_blank()) 
graf_total = chile_1 + theme_bw() + theme(legend.position = "none", axis.title = element_text(size=15)) +scale_fill_distiller(palette = "RdBu", direction = 1,limits=c(45,65)) 

chile_2 = ggplot() +  geom_sf(data = x %>% filter(Zoom==0), aes(fill=Tasa_Ingreso, geometry=geometry)) +  geom_sf_label_repel(data=x %>% filter(Zoom==0),aes(label = Region), nudge_x = 3, nudge_y = 0, seed = 10 ,colour="black", size=5) + coord_sf(xlim = c( -77.1,-65), ylim = c(-57, -38))
chile_2 = chile_2 + theme(legend.position="right", legend.box = "vertical") + labs(fill="Enrolment Rate")
chile_2 = chile_2 + xlab("Longitude") + ylab("Latitude") + annotation_scale() + annotation_north_arrow(location='tr')
chile_2 = chile_2 + theme(plot.title = element_text(hjust = 0.5),axis.title = element_text(size=15), legend.text = element_text(size=15)) + scale_fill_distiller(palette = "RdBu", direction=1,limits=c(45,65))  # + theme(legend.title=element_blank()) 
graf_total2 = chile_2 + theme_bw() + theme(legend.position = "right", axis.title = element_text(size=15), legend.text = element_text(size=12), legend.title = element_text(size=15)) +scale_fill_distiller(palette = "RdBu", direction=1,limits=c(45,65)) 

Fig_13 <- grid.arrange(bar,graf_total,graf_total2, ncol=3, nrow=1)


ggsave("/Users/bolea/Documents/GitHub/HE_Chile/Figures/Fig_13.eps",Fig_13, width=19.20, height=10.57, limitsize = FALSE)
ggsave("/Users/bolea/Documents/GitHub/HE_Chile/Figures/Fig_13.pdf",Fig_13, width=19.20, height=10.57, limitsize = FALSE)

#knitr::kable(head(colegios,100))  #LO MAS BAJO SEA ROJO Y ALTO AZUL

```

```{r Case of Schools - Fig: 8, 9, 10, 15, 16 and 17, echo=FALSE, fig.align="center", fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
library(rpart)
library(dplyr)
library(readr)
library(rattle)
  ...
  mutate(
    TIPO_INST_2=
      case_when(
        ...
        TIPO_INST_2 == "Universidades CRUCH" ~ "Traditional CRUCH",
        TIPO_INST_2 == "Universidades Privadas" ~ "Private Non-Traditional"
      )
  )
...
Count.Depe=
  ...
  as.data.frame() %>% 
  mutate(All=Private+Public+`Semi-Private`) %>% 
  melt(variable.name="DEPE",value.name="COUNT") %>% 
  mutate(AGNO=rep(2008:2020,4))

ggplot(Count.Depe) + aes(x=AGNO,y=COUNT,group=DEPE) +
  geom_line(aes(color=DEPE)) + 
  geom_point(aes(color=DEPE)) + 
  geom_text(aes(label=COUNT),vjust=-0.5,hjust=0,size=4,
            color="black",check_overlap=TRUE,nudge_y=0.5) +
  scale_color_discrete("Dependence") +
  theme_bw() + 
  scale_x_continuous(breaks=2008:2020) +
  scale_y_continuous(limits=c(0,3000)) + 
  labs(x="Admission Process",y="Number of high school") +
  theme(legend.position="top",
...
ggplot(Tipo_Institucion %>% filter(TIPO_INST_2!="No Matriculado")) + 
  aes(x=AGNO_EGRESO,y=N_distintas,group=TIPO_INST_2) +
  geom_line(aes(color=TIPO_INST_2)) +
  geom_point(aes(color=TIPO_INST_2)) +
  geom_text(aes(label=N_distintas),vjust=-0.5,hjust=0,size=4,
            color="black",check_overlap=TRUE,nudge_y=0.5) +
  scale_color_discrete("Institutions") + #labels=c("All")
  theme_bw() + 
  scale_x_continuous(breaks=2008:2020) +
  scale_y_continuous(limits=c(0,80)) +
  labs(x="Admission Process",y="Number of institutions") +
